...
`syncWindowConfig` 用于设置当前 `project_id` 的热窗口长度。如果统一服务没有同步
该项目的窗口配置，则默认使用 3 天。

该配置不会根据单次 `ingestData` 请求中的点数变化，生产接入请求也不再携带窗口
长度。不同项目可以在同一个 Core 实例中使用不同窗口长度；Core 按
`project_id` 维护独立的窗口、配置和 TDengine 原始数据范围。
...
序列在同一批次中写入。冷写默认使用 4 条 TDengine 写连接；Core 进程首次看到一个
`sequence_id` 时按轮询方式分配固定 writer，之后同一序列不会在多个 writer 之间迁移。
同一批次仍按 `sequence_id` 分组后投递到对应连接，独立序列可以并行写入。
