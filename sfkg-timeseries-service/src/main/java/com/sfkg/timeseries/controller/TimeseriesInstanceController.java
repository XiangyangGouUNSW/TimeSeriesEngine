...
public class TimeseriesInstanceController {
    ...
    public ApiResult<Void> queryInstanceConfigs() {
        instanceService.queryInstanceConfigs(null);
        return returnSuccess("timeseries instance query success");
    }

    @PostMapping(value = "/query", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ApiResult<Void> queryInstanceConfigsByJson(@RequestBody InstanceConfigQueryRequest request) {
        instanceService.queryInstanceConfigs(request);
        return returnSuccess("timeseries instance query success");
    }
}
