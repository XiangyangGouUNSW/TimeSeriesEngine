...
export const pages = {
  ...
  constraints: {
    ...
    fields: [
      ...
      { name: 'constraintName', label: '约束名称', type: 'text', required: true },
      {
        ...
        required: true,
        placeholder: '如 x < 40',
        hint: '表达式中的变量（如 x）需在下方约束项中定义变量名并绑定映射目标（实例或类别）',
      },
      {
        name: 'terms',
        label: '约束项 terms',
        type: 'terms',
        refType: 'instance',
        categoryRefType: 'category',
        required: true,
        default: [],
        full: true,
        hint: '选择约束级别（实例级/类别级），逐项配置变量名、映射目标、系数、采样偏移；变量映射自动生成',
      },
  ...
  forecastTasks: {
    ...
    columns: [
      ...
      { key: 'featureSequenceIds', label: '特征序列' },
      { key: 'forecastHorizon', label: '步长' },
      { key: 'modelKey', label: '模型' },
      { key: 'constraintIds', label: '约束ID' },
      { key: 'warningRule', label: '危险等级' },
      { key: 'status', label: '状态' },
      { key: 'updateTime', label: '更新时间' },
    ],
  },
